...
@section('title', 'Kelola Kategori FAQ - Admin Panel')
...
<div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
    ...
    <div class="flex items-center justify-between animate-on-scroll-bounce">
        <h1 class="text-3xl font-bold text-white animate-on-scroll">Kelola Kategori FAQ</h1>
    </div>
    ...
    <div class="bg-gradient-to-r from-gray-900/50 via-gray-800/50 to-gray-900/50 border border-gray-700/30 rounded-xl p-6 backdrop-blur-sm animate-on-scroll-zoom">
            ...
            <form action="{{ route('admin.faq-categories.store') }}" method="POST">
                ...
                <div class="flex flex-col sm:flex-row justify-end items-start sm:items-center gap-4 mt-6 sm:mt-8">
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto sm:ml-auto animate-on-scroll-zoom">
                        <a href="{{ route('admin.faqs.index') }}" class="text-white px-4 sm:px-6 py-2 sm:py-3 rounded-full font-medium transition-all duration-200 text-sm sm:text-base w-full sm:w-auto">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-3 animate-on-scroll-zoom animate-on-scroll-delay-1">
                                    <button 
                                        ...
                                        title="Hapus"
                                        data-url="{{ route('admin.faq-categories.destroy', $category) }}"
                                        data-item-name="kategori FAQ '{{ $category->name }}'"
                                        data-message="Apakah Anda yakin ingin menghapus kategori FAQ '{{ $category->name }}'? Semua FAQ yang terkait dengan kategori ini akan terpengaruh."
                                    >
        ...
        @else
            <div class="text-center py-8 animate-on-scroll-zoom">
                ...
                <p class="text-gray-400">Belum ada kategori FAQ yang tersedia</p>
            </div>
        @endif
    </div>
</div>
