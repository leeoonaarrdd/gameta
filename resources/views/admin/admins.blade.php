...
@section('title', 'Kelola Admin - Admin Panel')
...
<div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-8">
        ...
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-3xl font-bold text-white">Kelola Admin</h1>
            <button 
                id="addAdminBtn"
        ...
        <div id="adminModal" class="fixed inset-0 z-50 overflow-y-auto hidden">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="relative inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-gradient-to-r from-gray-950 via-gray-900 to-gray-950 backdrop-blur-sm border border-gray-700/30 rounded-2xl shadow-xl">
                    <!-- Modal header -->
                    <div class="flex items-center justify-between mb-6">
                        <h3 id="modalTitle" class="text-lg font-semibold text-white">Tambah Admin Baru</h3>
                    ...
                    <div class="mb-6 space-y-4">
                        <div>
                            <label for="modalAdminName" class="block text-sm font-medium text-gray-300 mb-2">Nama Lengkap</label>
                            <input 
                                type="text" 
                                id="modalAdminName"
                        ...
                        <div>
                            <label for="modalAdminUsername" class="block text-sm font-medium text-gray-300 mb-2">Username</label>
                            <input 
                                type="text" 
                        ...
                        <div>
                            <label for="modalAdminStatus" class="block text-sm font-medium text-gray-300 mb-2">Status</label>
                            <select 
                                id="modalAdminStatus"
                        ...
                        <div>
                            <label for="modalAdminPassword" class="block text-sm font-medium text-gray-300 mb-2">Password</label>
                            <input 
                                type="password" 
                        ...
                        <div>
                            <label for="modalAdminPasswordConfirmation" class="block text-sm font-medium text-gray-300 mb-2">Konfirmasi Password</label>
                            <input 
                                type="password" 
                                id="modalAdminPasswordConfirmation"
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full bg-gray-800/30 backdrop-blur-sm rounded-lg border border-gray-700/30">
            ...
            <tbody class="divide-y divide-gray-700/30">
                @include('admin.admins.partials.table-rows')
            </tbody>
        </table>
    </div>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mt-6 gap-4">
        <div class="text-sm text-gray-300 bg-gray-800/30 backdrop-blur-sm border border-gray-700/30 rounded-lg px-4 py-2">
            ...
            <span class="text-white font-semibold">{{ $admins->firstItem() ?? 0 }}</span> 
            <span class="text-purple-400 font-medium">to</span> 
            <span class="text-white font-semibold">{{ $admins->lastItem() ?? 0 }}</span> 
            <span class="text-purple-400 font-medium">of</span> 
            <span class="text-white font-semibold">{{ $admins->total() }}</span> 
            <span class="text-purple-400 font-medium">entries</span>
        </div>
        
        <div class="flex items-center gap-2 pagination-container">
            @if($admins->hasPages())
                {{ $admins->appends(request()->query())->links('vendor.pagination.tailwind') }}
            @else
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.loadAdmins = function() {
        ...
        // Make AJAX request
        fetch(`/admin/admins?${params.toString()}`, {
            headers: {
                ...
                if (confirm(message)) {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            if (typeof AdminPage !== 'undefined' && AdminPage.showNotification) {
                                AdminPage.showNotification(data.message, 'success');
                            } else {
                                alert(data.message);
                            }
                            loadAdmins(); // Reload data after successful deletion
                        } else {
                            if (typeof AdminPage !== 'undefined' && AdminPage.showNotification) {
                                AdminPage.showNotification('Gagal menghapus ' + itemName + ': ' + data.message, 'error');
                            } else {
                    ...
                    .catch(error => {
                        console.error('Error:', error);
                        if (typeof AdminPage !== 'undefined' && AdminPage.showNotification) {
                            AdminPage.showNotification('Terjadi kesalahan saat menghapus ' + itemName, 'error');
                        } else {
